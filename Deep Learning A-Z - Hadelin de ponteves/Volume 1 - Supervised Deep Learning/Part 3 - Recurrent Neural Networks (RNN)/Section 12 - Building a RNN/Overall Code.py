...
# Feature Scaling
from sklearn.preprocessing import MinMaxScaler    # From two options of Standardisation and Normalization, the scond one gives a better result. another reason for choosing the second option is because in LSTM we use several sigmoid activation function and since the sigmoid function takes values between 0 and 1 then we would rather to scale our values between 0 and 1 as it is the case in normalisation.
sc = MinMaxScaler()
